...
from backtester.validation.split import split_by_fraction

# Builds a signal from the return history available at the time: takes
# returns through date t, produces target weights aligned to the same index.
SignalBuilder = Callable[[pd.Series], pd.Series]
...
def out_of_sample_study(
    ...
    train_scores = {
        label: run_backtest(split.train, build(split.train), cost_model)
    ...
    in_sample = run_backtest(split.train, build(split.train), cost_model).summary()
